...
4. **`payload.ts`** — builds the single serializable object crossing the server/client boundary (`window.__REVIEW_DATA__`): shiki-highlighted hunks (`highlight.ts`) and pre-rendered mermaid SVGs.
...
- Runtime deps are only `beautiful-mermaid`, `shiki`, `zod`; the CLI bundles with `--packages=external`. Be reluctant to add dependencies.
